...
fn find_nearest_tabbed(
    ...
    match tree {
        ...
        Tiles::Array {
            ...
            nodes
                .iter()
                .find_map(|node| find_nearest_tabbed(node, tile_id, nearest))
                .or_else(|| {
                    floating_nodes
                        .iter()
                        .find_map(|floating| find_nearest_tabbed(&floating.tile, tile_id, nearest))
                })
        }
    }
}
    ...
    match &*tree {
        ...
        Tiles::Array {
            ...
            let selected = selected
                .filter(|selected| nodes.iter().any(|node| node_id(node) == *selected))
                .or_else(|| nodes.first().map(|node| node_id(node)));
        ...
        Tiles::Array {
            ...
            let floating_nodes = floating_nodes
                .iter()
                .map(|floating| {
                    Ok(Arc::new(FloatingTile {
                        tile: (*float_in_host(Arc::new(floating.tile.clone()), host_id, tile_id)?)
                            .clone(),
                        ..(**floating).clone()
                    }))
                })
...
fn wrap_root(tree: Arc<Tiles>, tile_id: TileId) -> Result<Arc<Tiles>, TilesStateError> {
    ...
    let selected = nodes.first().map(|node| node_id(node));
    Ok(Arc::new(Tiles::Array {
        id: TileId::new(),
...
fn extract_from_floating(
    ...
    let floating_nodes = floating_nodes
        ...
        .filter_map(|floating| {
            ...
            let (tile, found) = extract(Arc::new(floating.tile.clone()), tile_id);
            extracted = found;
            tile.map(|tile| {
    ...
    match &*tree {
        ...
        Tiles::Array {
            ...
            let Some(index) = floating_nodes
                .iter()
                .position(|floating| node_id(&floating.tile) == floating_id)
            else {
                return tree;
            };
...
mod tests {
    ...
    fn floating_the_only_root_tile_wraps_it_and_adds_a_default_tab() {
        ...
        assert_eq!(TileId::for_test(1), node_id(&floating_nodes[0].tile));
    }
    ...
    fn floating_uses_the_nearest_tabbed_parent() {
        ...
        assert_eq!(1, nodes.len());
        assert_eq!(TileId::for_test(2), node_id(&nodes[0]));
        assert_eq!(TileId::for_test(1), node_id(&floating_nodes[0].tile));
    }
