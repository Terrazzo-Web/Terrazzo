...
#[derive(Clone)]
pub struct XTemplate(Prc<TemplateInner>);
...
mod inner {
    ...
    pub struct TemplateInner {
        ...
        pub(super) element_mut: Prc<Mutex<Element>>,
...
impl XTemplate {
    pub fn new(element_mut: Prc<Mutex<Element>>) -> Self {
        Self::with_depth(Depth::zero(), element_mut)
    }

    pub(crate) fn with_depth(depth: Depth, element_mut: Prc<Mutex<Element>>) -> Self {
        use std::sync::atomic::AtomicI32;
        use std::sync::atomic::Ordering::SeqCst;
        static NEXT: AtomicI32 = AtomicI32::new(0);
        Self(Prc::new(TemplateInner {
