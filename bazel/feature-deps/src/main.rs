...
fn render_expression(child_features: &BTreeSet<String>, dependencies: &BTreeSet<String>) -> String {
    ...
    if !dependencies.is_empty() || parts.is_empty() {
        let deps = dependencies
            .iter()
            .map(|dependency| format!("{:?}", format_dependency_label(dependency)))
            .collect::<Vec<_>>()
            .join(", ");
...
fn format_dependency_label(dependency: &str) -> String {
    match dependency {
        "terrazzo-pty" => "//pty".to_owned(),
        "trz-gateway-client" => "//remote/client".to_owned(),
        "trz-gateway-common" => "//remote/common".to_owned(),
        "trz-gateway-server" => "//remote/server:acme".to_owned(),
        dependency => format!("@crates//:{dependency}"),
    }
}
...
"#,
    ...
    #[test]
    fn maps_workspace_dependencies_to_local_labels() {
        let features = parse_features(
            r#"
[features]
